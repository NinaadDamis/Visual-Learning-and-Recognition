...
class BaselineNet(nn.Module):
    ...
    def __init__(self, n_answers=5217):
        ...
        super().__init__()
        # Text encoder
        print("Initializing baseline net ---- ")
        t_type = "roberta-base"
        ...
        # Classifier
        print("Classifier input shape = ", self.text_encoder.config.hidden_size + 512)
    ...
    def forward(self, image, question):
        ...
        # print("Forward pass baseline net ----")
        text_features   = self.compute_text_feats(question)
        img_features    = self.compute_vis_feats(image)
        concat_features = torch.cat((img_features,text_features),dim=1)
        # print("Concat features shape = ", concat_features.shape)
        classifier_out  = self.classifier(concat_features)
        # print("Forward pass baseline net ---- end ")
    ...
    def compute_text_feats(self, text):
        ...
        # print("Text encoder out shape = ", encoded_text.pooler_output.shape)
        return encoded_text.pooler_output

    @torch.no_grad()
    def compute_vis_feats(self, image):
        """Convert image tensors to feature tensors."""
        img_out = self.vis_encoder(image)
        # print("Output shape vis_encoder = ", img_out.shape) # torch.Size([256, 512, 7, 7])
        pool_out = self.avgpool(img_out)
        # print("Pool shape  = ", pool_out.shape) # torch.Size([256, 512, 7, 7])
        pool_out = torch.squeeze(pool_out)
        # print("reshaped = ", pool_out.shape)
        return pool_out  # feed to vis_encoder and then mean pool on spatial dims
...
class CrossAttentionLayer(nn.Module):
    ...
    def __init__(self, d_model=256, dropout=0.1, n_heads=8):
        ...
        # FFN for seq1
        # Linear, relu, drop,linear,drop, drop + input , layer norm
        self.ffn_12 = nn.Sequential(nn.Linear(d_model,1024), nn.ReLU(),nn.Dropout(dropout),nn.Linear(1024,d_model),nn.Dropout(dropout))
        ...
        # FFN for seq2
        self.ffn_21 = nn.Sequential(nn.Linear(d_model,1024), nn.ReLU(),nn.Dropout(dropout),nn.Linear(1024,d_model),nn.Dropout(dropout))  # hidden dim is 1024
    ...
    def forward(self, seq1, seq1_key_padding_mask, seq2,
        ...
        # print("Seq1 seq2 shape = ", seq1.shape,seq2.shape)
        # Self-attention for seq1
        q1 = k1 = v1 = seq1
        ...
        # print("Cross 1b shape = ", cross1b.shape)
        norm_out_seq1 = self.norm_12(seq1 + self.dropout_12(cross1b))
        linear_out_seq1 = self.ffn_12(norm_out_seq1)
        # Add norm + linear before final norm
        final_seq1 = self.norm_122(linear_out_seq1 + norm_out_seq1)

        # Cross-attention from seq2 to seq1 and FFN
        cross2b = self.cross_21(
            query=q2,
            key=k1,
            value=v1,
            attn_mask=None,
        ...
        norm_out_seq2 = self.norm_21(seq2 + self.dropout_21(cross2b))
        linear_out_seq2 = self.ffn_21(norm_out_seq2)
        final_seq2 = self.norm_212(linear_out_seq2 + norm_out_seq2)
        # print("Shapes sq1 , seq2 ", seq1.shape, seq2.shape)
        seq1 = final_seq1
        seq2 = final_seq2
        return seq1, seq2
